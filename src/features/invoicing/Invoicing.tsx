...
function PdfActions({ invoice, autoArchive = false }: { invoice: FinalizedInvoice; autoArchive?: boolean }) {
  ...
  return <section className="pdf-actions" aria-label="PDF nota"><button className="button primary" disabled={busy} type="button" onClick={share}>{busy ? "Menyiapkan PDF…" : "Bagikan PDF"}</button><button className="button secondary" disabled={busy} type="button" onClick={showPreview}>Lihat PDF</button>{message && <p className="notice" role="status">{message}</p>}{preview && <div className="pdf-preview"><iframe title={`Pratinjau ${invoice.invoiceNumber}`} src={preview} /><button className="secondary-button" type="button" onClick={() => { URL.revokeObjectURL(preview); setPreview(null); }}>Tutup Pratinjau</button></div>}</section>;
}
